...
DO $$
    ...
    BEGIN
`
for (const [groupLabel, data] of Object.entries(tag_groups)) {
    query += `\t-- ${groupLabel} --\n`
    query += `\tINSERT INTO main.tag_groups (label) VALUES ('${groupLabel}') RETURNING tag_group_id INTO tgid;\n`

    for (const [locale, label] of Object.entries(data.l10n)) {
        query += `\tINSERT INTO main.tag_groups_l10n (tag_group_id, locale_id, data) VALUES (tgid, ${locale}_locale, '{"label": "${label}"}');\n`
    }

    for (const tag of new Set(data.tags.any)) {
        query += `\tINSERT INTO main.tags (label, tag_group_id) VALUES ('${tag.replace('\'', '\'\'')}', tgid);\n`
    }
}
...
fs.writeFileSync('./tags.sql', query, {
    encoding: 'utf8',
})
