...
class Like extends Method {
    async run(req: Req, user?: any): Promise<MethodRes> {
        ...
        const likeStatus = await this.query('getLikeStatus', {ideaId, userId}, {
            returnType: QueryReturnType.Row,
        })

        if (likeStatus && likeStatus.exists && likeStatus.dislike != dislike) {
            await this.query('unlike', {ideaId, userId})
        ...
        await this.query(queryName, {ideaId, userId, dislike})

        // FIXME: Simplify -- remove query
        const updatedStatus = await this.query('getLikeStatus', {ideaId, userId}, {
            returnType: QueryReturnType.Row,
        })
